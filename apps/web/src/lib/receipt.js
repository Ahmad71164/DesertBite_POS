export function buildReceiptHtml(data) {
    ...
    const fmt = (n) => `${sym} ${Math.round(n).toLocaleString("en-PK")}`;
...
export function printReceiptWindow(data, autoPrint = true, options) {
    ...
    const popup = window.open("", "desert-bite-receipt", "width=460,height=700,left=200,top=80,scrollbars=yes,resizable=yes");
    if (!popup) {
